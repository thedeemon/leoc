...
let atype = (tok Lbyte >>= fun _ -> return Leo.AByte) ||| (tok Ltint >>= fun _ -> return Leo.AInt) 
...
let params = tok Llparen >>> p_list ident (tok Lcomma) >>= fun ps -> tok Lrparen >>= fun _ -> return ps
...
and simple_expr_r s = ( 
	log "simple_expr"; (
	    (tok Lpipe >>> expr >>= fun e -> tok Lpipe >>= fun _ -> return (Leo.Length e))
	||| (tok Ldo >>> opt_terms >>> stmts >>= fun code -> opt_terms >>> tok Lend >>= fun _ -> return (Leo.Comp code))	
	||| (tok Llcurly >>> opt_terms >>> stmts >>= fun code -> opt_terms >>> tok Lrcurly >>= fun _ -> return (Leo.Comp code))
	||| (tok Lif >>> condition >>= fun con -> then_ >>> expr >>= fun e1 -> 
					p_opt None (else_ >>> expr >>= fun e2 -> return (Some e2)) >>= fun e2o -> return (Leo.If(con, e1, e2o)) )
	||| (tok Lnew >>> atype >>= fun ty -> tok Llbracket >>> expr >>= fun e -> tok Lrbracket >>= fun _ -> return (Leo.New(ty, e)))
	||| (tok Lbackslash >>> p_list ident (tok Lcomma) >>= fun ps -> tok Lfollow >>> expr >>= fun e -> return(Leo.Lambda(ps, e)))
	||| (ident >>= fun name -> tok Llparen >>> args >>= fun es -> tok Lrparen >>= fun _ -> 
				return (if name="print" then Leo.Comp [Leo.Print(List.hd es)] else Leo.Call(name, es)))	
	||| (ident >>= fun name -> tok Llbracket >>> expr >>= fun e1 -> tok Ldot2 >>> expr >>= fun e2 -> tok Lrbracket >>= fun _ ->
				return (Leo.SubArr(name, Leo.SRange(e1, e2))))
	||| (ident >>= fun name -> tok Llbracket >>> expr >>= fun e -> tok Lrbracket >>= fun _ ->	return (Leo.Arr(name, e)))
	||| (ident >>= fun name -> return (Leo.Var name))
	||| (int_ >>= fun i -> return (Leo.Val i))
	||| (tok Llparen >>> expr >>= fun e -> tok Lrparen >>= fun _ -> return e)
	)) s			
...
and value s = (
	log "value";
			(simple_expr >>= fun e -> tok Lhead >>= fun _-> return (Leo.Head(e)))
	|||	(simple_expr >>= fun e -> tok Ltail >>= fun _-> return (Leo.Tail(e)))				
	||| simple_expr				
	) s
...
and stmt_r s = (
	...
	(tok Lfor >>> p_list name_seq (tok Lcomma) >>= fun nsl -> opt_terms >>> stmts >>= fun code ->
		 opt_terms >>> tok Lend >>= fun _ -> return (Leo.For(nsl, code)))
...
and lvalue_r s = (
	log "lvalue";
	    (ident >>= fun name -> tok Llbracket >>> expr >>= fun e -> tok Lrbracket >>= fun _ -> return (Leo.LArr(name, e)))
	||| (ident >>= fun name -> tok Llbracket >>> seq >>= fun sq -> tok Lrbracket >>= fun _ -> return (Leo.LSubArr(name, sq)))
	||| (ident >>= fun name -> return (Leo.LVar name))	
	) s			
			
and seq s = (
	log "seq";
	(expr >>= fun e1 -> tok Ldot2 >>> expr >>= fun e2 -> return (Leo.SRange(e1, e2)))
	||| (expr >>= fun e1 -> tok Lrange >>= fun _ -> return (Leo.SRange(Leo.Val 0, Leo.Arith(Sub, Leo.Length e1, Leo.Val 1))))
...
and comparison s = (
	...
	||| (tok Llparen >>> condition >>= fun c -> tok Lrparen >>= fun _ -> return c)
	) s		
